...
def test_reader_create_request_success(client, reader_token, editor_user, auth_header):
    resp = client.post(
        "/reader/requests",
        json={"editor_id": editor_user.id, "conteudo": "Gostaria de acesso ao catalogo"},
...
def test_reader_routes_forbid_non_reader(client, editor_token, editor_user, auth_header):
    resp = client.post(
        "/reader/requests",
        json={"editor_id": editor_user.id, "conteudo": "teste"},
