...
export function ChatPage() {
  ...
  const {
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam }) => api<PaginatedResponse<Conversation>>(`/reader/conversations?page=${pageParam}&per_page=15`),
  ...
  // Intersection Observer for Conversations
  useEffect(() => {
  ...
  return (
    <div className={`chat-shell ${activeUserId ? 'has-active-chat' : ''}`}>
      ...
      <aside className="chat-sidebar card" style={{ borderRadius: '24px', overflow: 'hidden' }}>
        ...
        <div className="chat-sidebar-list" style={{ padding: '12px' }}>
          {allConversations.map(conv => (
            ...
            >
              <div className="avatar-circle chat-avatar" style={{ border: 'none', background: 'var(--primary-soft)', color: 'var(--primary)', fontWeight: 800 }}>
              ...
              <div className="chat-contact-info">
                <div className="chat-contact-top">
                  <span className="chat-contact-name" style={{ fontWeight: 800 }}>{conv.user_nome}</span>
                  {conv.unread_count > 0 && <span className="pill success mini-pill" style={{ height: '20px', minHeight: '20px', padding: '0 8px', fontSize: '10px' }}>{conv.unread_count}</span>}
                </div>
          ...
          <div ref={loadMoreConvRef} style={{ padding: '20px', textAlign: 'center' }}>
            ...
            {!hasNextPage && allConversations.length > 0 && <p className="muted small" style={{ fontSize: '11px', fontWeight: 600 }}>Fim da lista</p>}
          </div>

          {allConversations.length === 0 && !isConvLoading && (
            <div style={{ padding: '40px 20px', textAlign: 'center' }}>
               <p className="muted" style={{ fontSize: '14px' }}>Nenhuma conversa iniciada.</p>
            </div>
          )}
        </div>
      </aside>
      ...
      <main className="chat-main card" style={{ borderRadius: '24px', overflow: 'hidden' }}>
        {activeUserId ? (
          <>
            <div className="chat-header" style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(10px)' }}>
              <button className="btn secondary chat-back-btn" style={{ border: 'none', background: 'transparent' }} onClick={() => navigate('/mensagens')}>
                ⬅️
              </button>
              <div className="avatar-circle chat-header-avatar" style={{ width: '40px', height: '40px', border: 'none', background: 'var(--primary-soft)', color: 'var(--primary)', fontWeight: 800 }}>
                 {activeUser?.user_imagem_url ? <img src={activeUser.user_imagem_url} alt={activeUser.user_nome} /> : <span>{activeUser?.user_nome.slice(0,1).toUpperCase() || '💬'}</span>}
              </div>
              <div className="chat-header-info">
                <strong style={{ fontSize: '1.1rem' }}>{activeUser?.user_nome || 'Conversa'}</strong>
                <Link to={`/perfil/${activeUserId}`} className="link-hover muted" style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' }}>Ver perfil</Link>
              </div>
            </div>
            ...
            <div ref={scrollRef} className="chat-messages" style={{ padding: '24px', background: 'var(--background)' }}>
              {messagesQ.data?.map(m => {
                ...
                return (
                  <div key={m.id} className={`chat-bubble ${isMe ? 'outgoing' : 'incoming'}`} style={{ 
                    maxWidth: '80%', 
          ...
          <div className="chat-empty-state" style={{ background: 'var(--background)' }}>
             <div className="chat-empty-icon" style={{ fontSize: '4rem', marginBottom: '16px' }}>💬</div>
             <h3 style={{ fontSize: '1.6rem', fontWeight: 900, letterSpacing: '-0.5px' }}>Suas Mensagens</h3>
             <p className="muted" style={{ maxWidth: '300px', margin: '0 auto', fontSize: '14px' }}>Selecione uma conversa para começar a trocar experiências literárias.</p>
          </div>
        )}
      </main>
    </div>
